...
* **Authority for `.env`:** `scripts/gen-env.sh` (only).
  ...
  * Values are produced via `envsubst`; source variables must be set before substitution. Leave spaces, hashes, or other shell metacharacters unescaped—the generator writes them verbatim.
